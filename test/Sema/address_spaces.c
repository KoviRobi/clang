         ...
         _AS1 float b) // expected-error {{parameter may not be qualified with an address space}}
{
  ...
  __attribute__((address_space(0x3FFFFF))) int *_boundsB;
  __attribute__((address_space(0x7FFFFF))) int *_boundsC; // expected-error {{address space is larger than the maximum supported}}
  __attribute__((address_space(0x1000000))) int *_boundsD; // expected-error {{address space is larger than the maximum supported}}
  // chosen specifically to overflow 32 bits and come out reasonable
  __attribute__((address_space(4294967500))) int *_boundsE; // expected-error {{address space is larger than the maximum supported}}
